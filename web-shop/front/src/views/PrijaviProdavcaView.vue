...
export default {
  ...
  methods: {
    ...
    PrijaviProdavca() {
      const id = this.$route.params.id;
      axios.post(`http://localhost:8081/prijavi-prodavca/${id}`, this.prijava, {withCredentials: true})
          .then((res) => {
            alert("Uspesno ste prijavili prodavca!");
            this.$router.push('/profiles');
          })
          .catch((error) => {
            console.log(error.data);
            alert("Ne mozete da prijavite prodavca!");
            this.$router.push('/profiles');
          });
    }
  }
...
<template>
  ...
  <div class="main">
    <h3>Prijavite prodavca: <br> {{korisnik.ime}} {{korisnik.prezime}} </h3>
